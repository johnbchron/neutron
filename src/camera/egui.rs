...
pub struct EguiWantsFocus {
  // Must track previous value too, because when you click inside an egui
  // window, Context::wants_pointer_input() returns false once before
  // returning true again, which is undesirable.
  prev: bool,
  curr: bool,
}

pub fn not_interfering_with_egui(status: Res<EguiWantsFocus>) -> bool {
  !(status.curr || status.prev)
}
